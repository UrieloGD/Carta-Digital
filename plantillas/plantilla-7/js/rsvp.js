...
function enviarConfirmacion() {
    ...
    fetch('./plantillas/plantilla-7/api/rsvp.php', {
        method: 'POST',
        body: formData
    })
...
function mostrarRespuestaExistente(grupo, respuestaExistente) {
    ...
    fetch(`./plantillas/plantilla-7/api/rsvp.php?action=cargar_respuesta&id_grupo=${grupoActual.id_grupo}`)
    .then(response => response.json())
    .then(data => {
        if (data.success) {
...
document.querySelector('.modal-content-rsvp')?.addEventListener('click', function(e) {
    e.stopPropagation();
});